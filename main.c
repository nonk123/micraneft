 ...
 */
void collide(conge_ctx* ctx, entity_t* entity, double* x1, double* y1)
{
  int x, y;
  ...
  for (y = 0; y < entity->height; y++)
    for (x = 0; x < entity->width; x++)
      {
        tile_t* tile = get_tile(entity->ix + x, entity->iy + y);
...
void run_physics(conge_ctx* ctx)
{
...
void render_world(conge_ctx* ctx)
{
  int x, y;
...
void render_ui(conge_ctx* ctx)
{
  ...
  int occupied = can_place_tile_at(cursor_wx, cursor_wy);

  conge_pixel* cursor = conge_get_pixel(ctx, cursor_sx, cursor_sy);
  cursor->character = 'X';
  cursor->fg = in_range ? CONGE_WHITE : CONGE_RED;
...
void accept_input(conge_ctx* ctx)
{
  int numeric;
  ...
  if (is_cursor_in_range(controls.cursor_x, controls.cursor_y))
    {
      ...
      if (controls.breaking && !controls.placing)
        ...
        place_tile_at(world_x, world_y, tile);
    }
